...
function determineDpi(scaleFactor) {
    const baseDpi = 72; // Standard DPI for 1x scale
...
function generateLayoutInternal(options, callback) {
    ...
    function createImages(img, callback) {
        const sharpOpts = { density: determineDpi(options.pixelRatio || 1) };

        if (options.maxIconSize) {
            sharpOpts.limitInputPixels = options.maxIconSize * options.maxIconSize;
        }

        try {
          let svgBuffer = Buffer.from(img.svg);

          sharp(svgBuffer, sharpOpts)
              .toBuffer((err, image, metadata) => {
                  if (err) {
                      const message = err.message || '';
                      if (message.match(/Input image exceeds pixel limit/) && options.removeOversizedIcons) {
                          return callback(null, null);
                      }
                      if (message.match(/Input file is missing/)) {
                          return callback(null, null);
                      }
                      if (message.match(/Input buffer has corrupt header/) && message.match(/bad dimensions/)) {
                          return callback(null, null);
                      }
                      return callback(err);
                  }
                  ...
                  if (
                      ...
                      // Empty pixel image.
                      (metadata.width === 1 && metadata.height === 1 && metadata.size === 91)
                  ) {
                      return callback(null, null);
                  }
...
function generateImage(layout, callback) {
    return generateOptimizedImage(layout, {}, callback);
}
...
function generateOptimizedImage(layout, options, callback) {
    ...
    if (!layout.items.length) {
        return callback(null, emptyPNG);
    }

    let { items, width, height } = layout;

    // Default quality if not provided in options
    let quality = options.quality || 256;
    const pixelRatio = layout.pixelRatio || 1;

    // Create a base image with a transparent background
    let baseImage = sharp({
        create: {
            width: width,
            height: height,
            channels: 4,
            density: determineDpi(pixelRatio),
            background: { r: 0, g: 0, b: 0, alpha: 0 } // transparent background
        }
    });

    // Prepare composite layers from input items
    let layers = items.map(item => ({
        input: item.buffer,
        top: item.y,
        left: item.x
    }));
